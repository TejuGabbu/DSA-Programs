...
class SizeSumMaximumAnd HeightOfABinaryTree {
...
class Solution 
{
    public int sizeOfTree(Node root) 
	{
		...
		int leftnodes = sizeOfTree(root.left);
		int rightnodes = sizeOfTree(root.right);
		int overall = leftnodes+rightnodes+1;
		return overall;
    }
    ...
    public int sumOfTree(Node root) 
	{
		...
		int fully = left+root.data+right;
		return fully;
		
    }

    public int maxOfTree(Node root) 
	{ // return the maximum int in this given tree
		if(root==null)
		{
			return 0;
		}
		...
		int max = Math.max(left,(Math.max(root.data,right)));
		return max;
	}
    ...
    public int heightOfTree(Node root)
	{
		...
		int over = Math.max(left,right)+1;
		return over;
    }
}